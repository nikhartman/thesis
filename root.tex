...
\include{chapter_front_matter}  % title, abstract, acknowledgement, dedication, TOC, figure list
\include{chapter_intro}  % an introduction to the field and some bullshit to tie the whole thing together
\include{chapter_electronic_properties_cnt} % derivation of CNT band structure, explanation of QD + measurements
\include{chapter_cnt_growth_placement} % growth of CNT, placement on substrate, imaging
\include{chapter_cnt_contacts} %barrier info, statistics on lithography/deposition methods
\include{chapter_measurement} % explanation of cryogenic and transport measurements used in this work
\include{chapter_cnt_fm} % describe results of FMCNTQD made in the ebeam evaporator that showed possibly interesting behavior.%
\include{chapter_fm_cnt_sc} % explore spin transport through a FM-CNT-SC junction
\include{chapter_other_devices} % discuss other projects
\include{chapter_conclusion} % discuss other projects
\appendix
\include{appendix_fabrication} % details about fabrication 
\include{appendix_cryostat_details} % details about measurement
